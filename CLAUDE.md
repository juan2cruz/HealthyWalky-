...
- CI (Codemagic) has no `.env` in the repo clone (`.env*` is gitignored, but pubspec declares `.env` as a required asset) — a pre-build script in the Codemagic UI must write it from secure env vars (`SUPABASE_URL`, `SUPABASE_ANON_KEY`).
